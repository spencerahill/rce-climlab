...
rule rce_single_lat:
    output:
        temp("tmp/albedo_" + str(config['albedo']) +
             "/solar_lon_{day_of_year}/lat_{lat}.nc")
    params:
        lat="{lat}",
        day_of_year="{day_of_year}"
    log:
        "logs/single_lat/albedo_" + str(config['albedo']) +
        "/solar_lon_{day_of_year}/lat_{lat}.log"
    benchmark:
        "benchmarks/single_lat/albedo_" + str(config['albedo']) +
...
rule rce_mult_lats:
    input:
        expand("tmp/albedo_" + str(config['albedo']) +
               "/solar_lon_{{day_of_year}}/lat_{lat}.nc", lat=LATS)
    output:
        protected("out/albedo_" + str(config['albedo']) +
                  "/solar_lon_{day_of_year}.nc")
    log:
        "logs/mult_lat/albedo_" + str(config['albedo']) +
        "/solar_lon_{day_of_year}.log"
    benchmark:
        "benchmarks/mult_lat/albedo_" + str(config['albedo']) +
...
rule rce_ann_cycle:
    input:
        expand("out/albedo_" + str(config['albedo']) + "/solar_lon_{day}.nc",
               day=DAYS_OF_YEAR)
    output:
        protected("out/albedo_{config.albedo}/ann_cycle.nc")
    run:
        import xarray as xr
